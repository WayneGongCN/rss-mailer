...
function handler(options: Options) {
  ...
  fetcher(conf)
    .then(res => {
      error.push(...res.filter(x => x.status === 'rejected').map((x: PromiseRejectedResult) => x.reason))
      return res.filter(x => x.status === 'fulfilled').map((x: PromiseFulfilledResult<FetcherRes>) => x.value)
    })

    .then(parser(conf))
    .then(res => {
      error.push(...res.filter(x => x.status === 'rejected').map((x: PromiseRejectedResult) => x.reason))
      return res.filter(x => x.status === 'fulfilled').map((x: PromiseFulfilledResult<PaseRes>) => x.value)
    })
